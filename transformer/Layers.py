...
class DecoderLayer(nn.Module):
    ...
    def forward(
            self, dec_input, enc_output, score_matrix,
            slf_attn_mask=None, dec_enc_attn_mask=None, cover=None):
        dec_output, dec_slf_attn, dec_hidden_states = self.slf_attn(
            dec_input, dec_input, dec_input, mask=slf_attn_mask, cover=None, score_matrix=None)
        dec_output, dec_enc_attn, enc_dec_contexts = self.enc_attn(
            dec_output, enc_output, enc_output, mask=dec_enc_attn_mask, cover=cover, score_matrix=score_matrix)
